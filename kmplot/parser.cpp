...
{	double erg;
	...
	else {errpos=lptr-(str.latin1())+1; return 0.;}                          
}
...
{	int ix, p1, p2, p3;
    ...
    if(p2==-1 || p2>p3) p2=p3;
    if(getfix(str.left(p1))!=-1) {err=8; return -1;}
    else err=0;
	...
	if(ix==ufanz) {err=5; return -1;}	// zu viele Funktionen
...
{	Ufkt **p=(Ufkt**)mptr;
    ...
    if(evalflg==0)
	{	if(mptr>=&mem[memsize-10]) err=6;
		else {*p++=adr; mptr=(unsigned char*)p;}
	}
...
{	switch(err)
	{  case 1:	KMessageBox::error(0, i18n("Parser error at position %1:\n"
	  		  	"Syntax error").arg(QString::number(errpos)), "KmPlot"); break;
	   case 2: 	KMessageBox::error(0, i18n("Parser error at position %1:\n"
				"Missing parenthesis").arg(QString::number(errpos)), "KmPlot"); break;
	   case 3: 	KMessageBox::error(0, i18n("Parser error at position %1:\n"
				"Function name unknown").arg(QString::number(errpos)), "KmPlot"); break;
	   ...
	   case 7: 	KMessageBox::error(0, i18n("Parser error at position %1:\n"
				"Stack overflow").arg(QString::number(errpos)), "KmPlot"); break;
       case 8: 	KMessageBox::error(0, i18n("Parser error at position %1:\n"
				"Name of function not free").arg(QString::number(errpos)), "KmPlot"); break;
	}
	return err;
}

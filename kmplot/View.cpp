...
{	char fktmode, p_mode;
    ...
    if(ps.fktext[ix].f_mode==0) return;	// NOPLOT
   	...
   	if(dmin==dmax)
   	{	if(fktmode=='r') {dmin=0.; dmax=2*M_PI;}
   		else {dmin=xmin; dmax=xmax;}
   	}
   	...
   	{	k=0; ke=ps.fktext[ix].k_anz;
		...
		{	ps.setparameter(ix,ps.fktext[ix].k_liste[k]);
			...
			{	int errno=0;
				switch(p_mode)
				{  case 0:	y=ps.fkt(ix, x); break;
				   case 1:	y=ps.a1fkt(ix, x); break;
				...
				{	if(mflg>=1) pDC->moveTo(xi, yi);
					else {pDC->lineTo(xi, yi); pDC->drawPoint(xi, yi);}
					mflg=1;
				}
				else
				{	if(mflg==2) pDC->moveTo(xi, yi);
					else {pDC->lineTo(xi, yi); pDC->drawPoint(xi, yi);}
					mflg=0;
				}
			}

...
{   int xi;                			// transformierte x-Koordinate
    ...
    if(x<xmin) {xclipflg=1; return PlotArea.left();}
    if(x>xmax) {xclipflg=1; return PlotArea.right();}
	xi=(int)(ox+skx*x);
    xclipflg=0;
    return xi;
}
...
{   int yi;                     	// transformierte y-Koordinate
    ...
    if(y<ymin) {yclipflg=1; return PlotArea.bottom();}
    if(y>ymax) {yclipflg=1; return PlotArea.top();}
    yi=(int)(oy-sky*y);
    yclipflg=0;
    return yi;
}
...
{	int a, b, tl;
    ...
    {	int dx, dy;
		...
		{	dx=(int)(0.4*skx); dy=(int)(0.15*sky);
		...
		{	dx=(int)(0.15*skx); dy=(int)(0.4*sky);
			pDC->Line(a, b, a-dx, b+dy);
			pDC->Line(a, b, a+dx, b+dy);
		}
    }
	...
	{	da=oy-TeilstrichLaenge;
    	...
    	d=tsx;
    	if(da>(double)PlotArea.bottom()) {a=PlotArea.bottom()-tl; b=PlotArea.bottom()+TeilstrichLaenge;}
    	else if(db<(double)PlotArea.top()) {b=PlotArea.top(); a=PlotArea.top()-TeilstrichLaenge;}
    	else {a=(int)da; b=(int)db;}
    	while(d<xmd-ex/2.) {pDC->Linev(Transx(d), a, b); d+=ex;}

    	da=ox-TeilstrichLaenge;
    	db=ox+TeilstrichLaenge;
    	d=tsy;
    	if(da<(double)PlotArea.left()) {a=PlotArea.left()-tl; b=PlotArea.left()+TeilstrichLaenge;}
    	else if(db>(double)PlotArea.right()) {b=PlotArea.right(); a=PlotArea.right()-TeilstrichLaenge;}
    	else {a=(int)da; b=(int)db;}
    	while(d<ymd-ey/2.) {pDC->Lineh(a, Transy(d), b); d+=ey;}
	}
	else if(mode&RAHMEN)
...
{	int a, b;
	...
	{	d=tsx;
		while(d<xmd) {pDC->Linev(Transx(d), PlotArea.bottom(), PlotArea.top()); d+=ex;}
		d=tsy;
		while(d<ymd) {pDC->Lineh(PlotArea.left(), Transy(d), PlotArea.right()); d+=ey;}
    }
    else if(g_mode==2)            		// Kreuzraster
    {	int dx, dy;

    	dx=(int)(0.05*skx); dy=(int)(0.05*sky);
    ...
    {	int x1, y1, x2, y2, dr, d2r, xm;
    	...
    	xm=(int)(c+ox);
    	dr=(int)(skx*ex); d2r=(int)(2.*skx*ex);
    	x1=(int)ox-dr; y1=(int)oy-dr;
    	x2=y2=d2r;
